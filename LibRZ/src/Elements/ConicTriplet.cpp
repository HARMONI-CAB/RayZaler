...
ConicTriplet::recalcModel()
{
  ...
  const Real Rmax1 = RZ::ConicSurface::Rmax(sigma[0], Rc[0], m_K[0], m_displacement[0], sigma[1], Rc[1], m_K[1], m_displacement[1], m_thickness1);
  const Real Rmax2 = RZ::ConicSurface::Rmax(sigma[1], Rc[1], m_K[1], m_displacement[1], sigma[2], Rc[2], m_K[2], m_displacement[2], m_thickness2);
  const Real Rmax3 = RZ::ConicSurface::Rmax(sigma[2], Rc[2], m_K[2], m_displacement[2], sigma[3], Rc[3], m_K[3], m_displacement[3], m_thickness3);
  ...
  if (rho > (Rmax - m_radius)) {
    ...
    if (rho < m_radius) {
        zSupVal = fmin(z_sup[0], fmin(z_sup[1], fmin(z_sup[2], z_sup[3]))) - (.5 * m_thickness2 + m_thickness3);
        zInfVal = fmax(z_inf[0], fmax(z_inf[1], fmax(z_inf[2], z_inf[3]))) + (.5 * m_thickness2 + m_thickness1);
      } else {
        zSupVal = fmin(z_sup[1], fmin(z_sup[2], z_sup[3])) - (.5 * m_thickness2 + m_thickness3);
        zInfVal = fmax(z_inf[1], fmax(z_inf[2], z_inf[3])) + (.5 * m_thickness2 + m_thickness1);
      }
